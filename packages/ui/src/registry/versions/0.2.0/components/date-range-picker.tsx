...
export function DateRangePicker({
	...
	const triggerStyles = clsx(
		"flex w-full items-center justify-between gap-2 rounded-rounded-md border bg-surface-1 px-3 py-2",
				...
				<Popover.Portal>
					<Popover.Positioner sideOffset={4}>
						...
						>
							<div
								className={twMerge(
									"mb-3 flex items-center gap-2 rounded-radius-sm bg-surface-2 p-2",
							...
							>
								<button
									...
									className="rounded-radius-sm p-1 transition-colors hover:bg-surface-2"
								>
									<ChevronLeft className="size-4" />
								</button>
								<span className="font-medium text-text-base text-typography-primary">
							...
							<div className="mb-3 grid grid-cols-7 gap-1">
								{weekDays.map((day) => (
									<div
										key={day}
										className="p-2 text-center text-text-xs text-typography-muted"
									>
										{day}
									</div>
								))}
											...
											className={twMerge(
												clsx(
													"rounded-radius-sm p-2 text-center text-text-sm transition-colors",
													{
														...
														"bg-primary text-primary-on": day.selected,
														"bg-primary-muted": isInRange && !day.selected,
														"text-typography-muted": day.disabled,
														"font-semibold": day.now,
													},
												),
												classNames?.day,
											)}
							...
							{selectedDates.length > 0 && (
								...
								>
									<button
										type="button"
										onClick={handleClear}
										className="rounded-radius-sm px-3 py-1.5 text-secondary text-text-sm transition-colors hover:bg-surface-2"
									>
										Clear
									</button>
								</div>
