...
export function FileUpload({
	...
	const dropzoneStyles = clsx(
		...
		"hover:border-primary hover:bg-surface-1-hover",
		{
			...
			"border-border": !isDragging && !error,
			"border-error": error,
	...
	return (
		...
		>
			...
			>
				<Upload className="mb-3 size-12 text-typography-muted" />
			...
			{value.length > 0 && (
				<div className={twMerge("flex flex-col gap-2", classNames?.fileList)}>
					{value.map((file, index) => (
						<div
							key={`${file.name}-${index}`}
							className="flex items-center justify-between gap-2 rounded-radius-sm border border-border bg-surface-1 p-2"
						>
							<div className="flex-1 truncate">
								<p className="truncate text-text-sm text-typography-primary">
									{file.name}
								</p>
							...
							<button
								...
								disabled={disabled}
								className="rounded-radius-sm p-1 text-typography-muted transition-colors hover:bg-surface-2 hover:text-error disabled:cursor-not-allowed disabled:opacity-50"
							>
								<X className="size-4" />
							</button>
						</div>
