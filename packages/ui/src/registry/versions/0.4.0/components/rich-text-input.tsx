...
export function RichTextInput({
	...
	const editor = useEditor({
		...
		editorProps: {
			attributes: {
				class: clsx(
					"min-h-32 w-full rounded-b-rounded-md border-x border-b bg-surface-1 px-3 py-2",
	...
	const toolbarButtonBase = clsx(
		"rounded-sm p-1.5 text-typography-secondary transition-colors",
	...
	return (
		...
		>
			...
			<div
				className={twMerge(
					clsx(
						"flex flex-wrap items-center gap-0.5 rounded-t-rounded-md border border-border bg-surface-1 p-1",
						{ "border-error": !!error },
					),
					classNames?.toolbar,
				)}
