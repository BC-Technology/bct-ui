				...
				<div className={twMerge("group relative", classNames?.root, className)}>
					{src ? (
						<img
							...
							alt={alt}
							className={twMerge(
								clsx("h-auto w-full rounded-md object-cover", {
						...
						<div className="flex h-24 w-full items-center justify-center rounded-md border border-border bg-surface-2 text-sm text-typography-muted">
							No image
						</div>
					)}
					...
					{showHoverButton && src && (
						<div className="pointer-events-none absolute top-1 right-1 opacity-0 transition-opacity group-hover:opacity-100">
							<button
								type="button"
								className="pointer-events-auto rounded-sm bg-surface-1 p-1 text-typography-primary shadow-shadow-sm transition-colors hover:bg-primary hover:text-primary-on"
								onClick={(e) => {
									e.stopPropagation()
									openViewer()
								}}
			...
			>
				<div className="flex w-full flex-col gap-4">
					{!hideMetadata && metadata && (
						<div
							className={twMerge(
								"flex flex-col gap-3 rounded-md border border-border bg-surface-2 p-4",
								classNames?.metadata,
							)}
						>
							{metadata.name && (
								<div className="flex items-center justify-between gap-2">
									<span className="text-sm text-typography-muted">Name</span>
									<span className="break-all text-right text-sm text-typography-primary">
										{metadata.name}
									</span>
								</div>
							)}
