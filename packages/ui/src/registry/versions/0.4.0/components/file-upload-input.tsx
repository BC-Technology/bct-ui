...
export function FileUploadInput({
	...
	const expandBtnClass =
		"absolute left-1 top-1 cursor-pointer rounded-sm bg-surface-1 p-0.5 text-typography-primary opacity-0 transition-all hover:bg-primary hover:text-primary-on group-hover:opacity-100"
	const removeBtnClass =
		"absolute right-1 top-1 cursor-pointer rounded-sm bg-surface-1 p-0.5 text-typography-primary opacity-0 transition-all hover:bg-error hover:text-error-on group-hover:opacity-100"
	const restoreBtnClass =
		"absolute right-1 top-1 cursor-pointer rounded-sm bg-surface-1 p-0.5 text-typography-primary opacity-0 transition-all hover:bg-success hover:text-success-on group-hover:opacity-100"
	...
	return (
		<div className={twMerge("w-full", classNames?.root, className)}>
			<button
				...
				className={twMerge(
					clsx(
						"w-full rounded-md border-2 border-dashed bg-surface-1 p-6 transition-colors duration-200 ease-in-out",
			...
			<div className="mt-2 flex items-center justify-between">
				{supportedText && (
					<span className="text-typography-muted text-xs">
					...
					{isImage ? (
							...
							{existingItems.map((item) => {
								...
								return (
									<div
										...
										className={clsx(
											"group relative aspect-square overflow-hidden rounded-md border border-border bg-surface-2",
											{ "opacity-30": removed },
										)}
									>
										{item.url ? (
							...
							{selectedFiles.map((file) => {
								...
								return (
									...
									>
										...
										<div className="absolute right-0 bottom-0 left-0 bg-linear-to-t from-overlay to-transparent p-2 opacity-0 transition-opacity group-hover:opacity-100">
											<p className="truncate text-light text-xs">{file.name}</p>
											<p className="text-light/80 text-xs">
						...
						<div className="flex flex-col gap-2">
							{existingItems.map((item) => {
								...
								return (
									<div
										key={`existing-${item.id}`}
										className={clsx(
											"flex items-center justify-between rounded-md bg-surface-1 p-2",
											{ "opacity-30": removed },
										)}
									>
							...
							{selectedFiles.map((file) => (
								<div
									key={`${file.name}-${file.lastModified}`}
									className="flex items-center justify-between rounded-md bg-surface-1 p-2"
								>
									<div className="flex items-center gap-3">
										<FileGenericIcon
											size={20}
			...
			{error && (
				<p className={twMerge("mt-2 text-error text-sm", classNames?.error)}>
					{error}
				</p>
			)}
			{ariaDescription && (
