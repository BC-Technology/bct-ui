			...
			{items.map((item) => (
				...
				>
					<BaseAccordion.Header className={twMerge(classNames?.header)}>
						<BaseAccordion.Trigger
							className={twMerge(
								clsx(
									...
									"font-medium text-base text-typography-primary transition-colors",
						...
						>
							{item.title}
							<ChevronDown className="size-4 shrink-0 text-typography-muted transition-transform duration-200 data-open:rotate-180" />
						</BaseAccordion.Trigger>
					</BaseAccordion.Header>
					<BaseAccordion.Panel
						className={twMerge(
							"overflow-hidden border-border border-t px-4 py-3 text-base text-typography-secondary",
							classNames?.panel,
						)}
					>
						{item.children}
