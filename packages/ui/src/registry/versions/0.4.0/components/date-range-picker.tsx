...
export function DateRangePicker({
	...
	const triggerStyles = clsx(
		"flex h-10 w-full items-center justify-between gap-2 rounded-md border bg-surface-1 px-3 py-2",
			...
			<Popover.Root>
				...
				<Popover.Portal>
					<Popover.Positioner sideOffset={6}>
						<Popover.Popup
							className={twMerge(
								"z-50 w-80 rounded-md border border-border bg-surface-1 p-4 shadow-shadow-lg",
								classNames?.popup,
							)}
						>
							<div
								className={twMerge(
									"mb-3 flex items-center gap-2 rounded-md bg-surface-2 p-2",
									classNames?.rangeDisplay,
								)}
							>
								<div className="flex-1 text-center">
									<p className="text-typography-muted text-xs">From</p>
									<p
								...
								<div className="flex-1 text-center">
									<p className="text-typography-muted text-xs">To</p>
									<p
										className={clsx("font-medium text-sm", {
							...
							>
								<button
									...
									className="rounded-sm p-1.5 text-typography-muted transition-colors hover:bg-surface-2 hover:text-typography-primary"
								>
									<ChevronLeft className="size-4" />
								</button>
								<span className="font-semibold text-base text-typography-primary">
							...
							<div className="mb-2 grid grid-cols-7 gap-1">
								{weekDays.map((day) => (
									<div
										key={day}
										className="p-1 text-center font-medium text-typography-muted text-xs"
									>
										{day}
									</div>
								))}
								...
								{calendar.days.map((day) => {
									...
									return (
										<button
											...
											className={twMerge(
												clsx(
													"rounded-sm p-2 text-center text-sm transition-colors",
													{
							...
							{selectedDates.length > 0 && (
								...
								>
									<button
										type="button"
										onClick={handleClear}
										className="rounded-sm px-3 py-1.5 text-sm text-typography-secondary transition-colors hover:bg-surface-2 hover:text-typography-primary"
									>
										Clear
									</button>
								</div>
