			...
			<BasePopover.Portal>
				<BasePopover.Positioner side={side} sideOffset={8}>
					<BasePopover.Popup
						className={twMerge(
							clsx(
								"z-popover w-72 rounded-md border border-border bg-surface-1 shadow-shadow-lg",
					...
					>
						{title && (
							<div className="flex items-center justify-between border-border border-b px-4 py-3">
								<BasePopover.Title
									className={twMerge(
										"font-semibold text-base text-typography-primary",
										classNames?.title,
									)}
								>
									{title}
								...
								<BasePopover.Close
									className={twMerge(
										clsx(
											"rounded-sm p-1 text-typography-muted transition-colors",
