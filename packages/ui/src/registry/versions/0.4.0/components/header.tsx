...
export function Header({
	...
	const navLinkClass = (isActive?: boolean) =>
		clsx(
			"flex items-center gap-3 rounded-md px-3 py-2.5 text-base text-typography-primary transition-all duration-200 ease-in-out",
					...
					<div className="flex h-14 w-full items-center justify-between px-4 py-2">
						...
						<div className="flex select-none items-center gap-3">
							...
							{title && (
								<>
									...
									<h3 className="font-semibold text-base text-typography-primary">
										{title}
									</h3>
								</>
							)}
						...
						<div className="flex items-center gap-2">
							...
							{user && (
								...
								>
									<Popover.Root>
										<Popover.Trigger
											className={clsx(
												"flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-typography-primary outline-none transition-all duration-200 ease-in-out",
												"hover:bg-surface-2",
													...
													<div className="px-3 py-2">
														<div className="flex items-center gap-3">
															...
															<div className="flex min-w-0 flex-1 flex-col">
																<p className="truncate font-medium text-sm text-typography-primary">
																	{user.name}
																</p>
																{user.email && (
																	<p className="truncate text-typography-muted text-xs">
																		{user.email}
																	</p>
																)}
															</div>
															...
															<div className="flex flex-col py-1">
																{onSettings && (
																	<button
																		...
																		onClick={onSettings}
																		className={clsx(
																			"flex w-full items-center gap-2 rounded-sm px-3 py-2 text-sm text-typography-primary transition-colors",
																			"hover:bg-surface-2",
																			"focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-focus",
																		)}
																	>
							...
							<button
								...
								className={clsx(
									"flex items-center justify-center rounded-md p-2 text-typography-primary transition-colors lg:hidden",
									"hover:bg-surface-2",
									"focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-focus",
								)}
								...
								{user && (
									<>
										...
										<button
											...
											className={clsx(
												"flex w-full items-center justify-between rounded-md px-3 py-2.5 text-typography-primary transition-all duration-200 ease-in-out",
											...
											<div className="ml-6 flex flex-col gap-1 pt-1">
												{onSettings && (
													<button
														...
														className={clsx(
															"flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-typography-primary transition-colors",
															"hover:bg-surface-2/50",
															"focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-focus",
														)}
													>
