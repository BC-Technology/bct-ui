...
/**
 * Try to load preview wrapper for a complex component
 */
export async function loadPreviewWrapper(
	version: string,
	componentName: string,
): Promise<VariantExample[] | null> {
	try {
		console.info(
			`Attempting to load wrapper for @/components/preview-wrappers/${version}/${componentName}-preview`,
		)
		const wrapper = await import(
			// We have to use a static string prefix for Next.js to properly bundle the dynamic imports
			`../components/preview-wrappers/${version}/${componentName}-preview.tsx`
		)
		console.info(
			`Successfully loaded wrapper! Has variants: ${!!wrapper.variants}`,
		)

		// Since these files are marked with "use client", Next.js replaces their exports with references
		// that cannot be invoked directly. For array exports, Next.js proxy objects might fail when mapped over
		// if they contain functions. Let's return the wrapper.variants and let the client handle it if possible.
		return Array.isArray(wrapper.variants) ? wrapper.variants : null
	} catch (error) {
		console.error(
			`Failed to load preview wrapper for ${componentName} v${version}:`,
			error,
		)
		return null
	}
 ...
 */
export async function generateVariantExamples(
	componentName: string,
	version?: string,
): Promise<VariantExample[]> {
	if (version) {
		const wrapperVariants = await loadPreviewWrapper(version, componentName)
		if (wrapperVariants) {
			console.info(
				`Found ${wrapperVariants?.length} wrapper variants for ${componentName}`,
			)
			return wrapperVariants
		}
		// Button variants
